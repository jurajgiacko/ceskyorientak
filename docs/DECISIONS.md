...
`krumlov` **30 640 336**, and it is the first Krumlov seed since D-032 that is
**chosen** rather than inherited. Top of 12 viable out of 200 menu-shaped
...
```
S→1     68/81    1.2x        8→9    119/127   1.1x
1→2    137/184   1.3x        9→10    49/52    1.1x
2→3     73/79    1.1x       10→11   126/157   1.2x
3→4     56/61    1.1x       11→12   100/108   1.1x
4→5    106/108   1.0x       12→13   209/320   1.5x
5→6     59/62    1.1x       13→14   112/124   1.1x
6→7     98/113   1.2x       14→F    359/738   2.1x
7→8     78/84    1.1x
total 1750 m straight, 2396 m walked — D 1.37
on the graph  1.3 1.5 1.2 1.6 1.1 1.2 1.5 1.2 1.2 1.2 1.2 1.1 1.7 1.1 2.3
```

| | |
|---|---|
| Controls · length · climb | 14 · 1750 m · 55 m (3.1 %) |
| Running on the street network | **95 %** |
| Control distance to a runnable way | median **0.0 m**, p90 4.3 m, worst 5.6 m |
| Start and finish off the network | **0.0 m** and **0.0 m** |
| Run-out toward control 1 | **34.5 m** before anything is met |
| Worst leg | **2.1×** against a 3.0× limit, taking the shorter of the two measures |
| Start to finish | 402 m |

The whole-course detour, D 1.37, is worse than `30_814_554`'s 1.14 and that is
...
**The pick was made twice, and the second one is the one that ships.** The first
ran against a build whose `pickNextControl` drew 200 bearings a leg; taking that
back to 90 for the load-time reason above re-rolls every course, because the
generator's RNG is drawn inside geometry-dependent branches (D-029). A seed
chosen on a build that is not the build is not a chosen seed — which is exactly
D-039's complaint about `COURSE_SEED` — so the whole two hundred were run again
on the build that ships.
...
| worst leg per candidate | D-037 (240) | phase 3, 200 draws | phase 3, **90 draws — shipped** |
|---|---|---|---|
| best | 1.49× | 1.36× | **1.30×** |
| p10 | 2.55× | 1.68× | **2.04×** |
| **median** | **8.71×** | 3.65× | **3.68×** |
| keeping every leg under 2.0× | 11 | 53 | **20** |
| keeping every leg under 3.0× | 39 (16 %) | 89 (45 %) | **77 (39 %)** |
| legs over 3.0×, of all legs | 524 of 3737 (14 %) | 173 of 3403 (5 %) | **219 of 3326 (7 %)** |

The middle column is worth keeping beside the third: **doubling the draws bought
six points of clean-candidate rate and moved the median not at all**, for twice
the loading screen. The candidate pool is not what was short.

D-037 concluded *"in this town roughly one seed in six is raceable, and finding
it is what a course setter is for."* It is now closer to two in five. The venue
...
```
 ~90  a leg over 3.0x on the 2 m router
  28  too few or too many controls
  20  the athlete runs too few metres out of the start   ← fault 8, filtered
  13  a leg over 3.0x on the street graph                ← §3, filtered
   9  a leg cannot be run at all
   6  took more than one seed to settle
   5  the start is on a bridge deck
```

Twenty candidates in two hundred put the start somewhere you cannot run out of. Every one of them would have passed every gate this project had before
